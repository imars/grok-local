...
def ask_local(command, ai_adapter, git_interface, debug=False, use_git=True, direct=False):
    ...
    if direct:
        # Direct execution mode with optional local inference fallback
        if re.match(r'^git\s+', command):
        ...
        elif re.match(r'^infer\s+', command):  # Explicit inference trigger
            start_bridge()
            req_id = str(uuid.uuid4())
            sub_command = command[6:].strip()  # Strip "infer "
            payload = {
                "input": (
            ...
            # Fallback to local inference if no strict match
            return execute_command(command, git_interface, ai_adapter, use_git)
    else:
        # Full inference mode: all commands go to the agent via bridge
        start_bridge()
        req_id = str(uuid.uuid4())
        payload = {
            "input": (
                f"Execute this command with your tool: '{command}'. "
                f"Tool: execute_command(command) - runs local commands (git, checkpoint, etc.) and returns results. "
                f"Restrictions: no external calls (e.g., weather, http) unless whitelisted. "
                f"Respond with the tool's output or explain why you can't proceed."
            ),
