...
# - 'Ensuring test script tests retry logic with changes' --git'
...
def get_git_file_tree() -> str:
    """Generate a tree-like structure of files not ignored by Git."""
    try:
        # Run git ls-files to get tracked and untracked-but-not-ignored files
        ...
        for file in files:
            parts = file.split('/')
            indent = "  " * (len(parts) - 1)  # Indent based on directory depth
